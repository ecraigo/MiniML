...
(* I hate appending ;; to everything... *)
let s_to_e s = str_to_exp (s ^ ";;") ;;
...
let repl () =
  ...
  while true do
	...
	| Evaluation.Env.Closure(_,_) -> raise
					   (Evaluation.EvalError "can't happen")
      with
      | Parsing.Parse_error -> printf "xx> parse error\n"
      | Evaluation.EvalError s -> printf "Error: %s\n" s
